...
#Filter Export(Klang)
klang_export = cargo_throughput_data.loc[:,cargo_throughput_data.columns.isin(['Date','Export(Klang)'])].reset_index(drop=True)
klang_export.head()
print("Export value in Klang")
print(klang_export)

#plot original time series data
plt.figure(figsize=(10,6))
plt.plot(klang_export['Date'], klang_export['Export(Klang)'])
plt.title('Cargo export value in Klang')
plt.xlabel('Date')
...
test_stationarity(klang_export,'Export(Klang)')

#autocorrelation factor
fig,ax= plt.subplots(2, figsize=(10,6))
ax[0] = plot_acf(klang_export['Export(Klang)'].dropna(), ax=ax[0], lags = 15)
ax[1] = plot_pacf(klang_export['Export(Klang)'].dropna(), ax=ax[1], lags = 15)
plt.show()

#decomposition
klang_export['Date'] = pd.to_datetime(klang_export['Date'])
klang_export_newdf = klang_export.set_index('Date')
decomp = sm.tsa.seasonal_decompose(klang_export_newdf['Export(Klang)'],model='additive')
decomp.plot()
plt.show()

#set traning and testing set
klang_export_train = klang_export[:round(len(klang_export)*70/100)]
klang_export_test = klang_export[round(len(klang_export)*70/100):]
...
for pm in pdq:
    for pm_seasonal in seasonal_pdq:
        try:
            model = sm.tsa.statespace.SARIMAX(klang_export_train['Export(Klang)'],
                                              order=pm,
...
model_klang_export = sm.tsa.statespace.SARIMAX(klang_export_train['Export(Klang)'],order=(1,1,1),seasonal_order=(0,1,1,12),
                                  enforce_stationarity=False,enforce_invertibility=False)
result = model_klang_export.fit()
...
klang_export_test['Date'] = pd.to_datetime(klang_export_test['Date'])
forecast_model = result.get_forecast(steps=len(klang_export_test))
predict_mean = forecast_model.predicted_mean
plt.figure(figsize=(10,6))
plt.plot(klang_export['Date'],klang_export['Export(Klang)'],label='Training Set')
plt.plot(klang_export_test['Date'],klang_export_test['Export(Klang)'],label='Testing Set')
plt.plot(klang_export_test['Date'], predict_mean,label='Forecast')
plt.title('Forecast Model of Klang Cargo Export Value')
plt.xlabel('Date')
plt.ylabel('\'000 Tan Metrik(Freightweight)')
plt.legend()
plt.show()
...
#check accuracy(RMSE)
actual_data = np.array(klang_export_test['Export(Klang)'])
...
#Plot time series
plt.figure(figsize=(10,6))
plt.plot(klang_export['Date'],klang_export['Export(Klang)'],label='Original Data')
