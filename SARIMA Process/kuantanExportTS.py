...
#Filter Export(Kuantan)
kuantan_export = cargo_throughput_data.loc[:,cargo_throughput_data.columns.isin(['Date','Export(Kuantan)'])].reset_index(drop=True)
kuantan_export.head()
print("Export value in Kuantan")
print(kuantan_export)

#plot original time series data
plt.figure(figsize=(10,6))
plt.plot(kuantan_export['Date'], kuantan_export['Export(Kuantan)'])
plt.title('Cargo export value in Kuantan')
plt.xlabel('Date')
plt.ylabel('\'000 Tan Metrik(Freightweight)')
plt.show()

#set traning and testing set
kuantan_export_train = kuantan_export[:round(len(kuantan_export)*70/100)]
kuantan_export_test = kuantan_export[round(len(kuantan_export)*70/100):]

#testing stationarity
test_stationarity(kuantan_export,'Export(Kuantan)')

#autocorrelation factor
fig,ax= plt.subplots(2, figsize=(10,6))
ax[0] = plot_acf(kuantan_export['Export(Kuantan)'].dropna(), ax=ax[0], lags = 15)
ax[1] = plot_pacf(kuantan_export['Export(Kuantan)'].dropna(), ax=ax[1], lags = 15)
plt.show()

#decomposition
kuantan_export['Date'] = pd.to_datetime(kuantan_export['Date'])
kuantan_export_newdf = kuantan_export.set_index('Date')
decomp = sm.tsa.seasonal_decompose(kuantan_export_newdf['Export(Kuantan)'],model='additive')
decomp.plot()
plt.show()
...
for pm in pdq:
    for pm_seasonal in seasonal_pdq:
        try:
            model = sm.tsa.statespace.SARIMAX(kuantan_export_train['Export(Kuantan)'],
                                              order=pm,
...
model_kuantan_export = sm.tsa.statespace.SARIMAX(kuantan_export_train['Export(Kuantan)'],order=(0,1,1),seasonal_order=(1,1,1,12),
                                  enforce_stationarity=False,enforce_invertibility=False)
result = model_kuantan_export.fit()
...
kuantan_export_test['Date'] = pd.to_datetime(kuantan_export_test['Date'])
forecast_model = result.get_forecast(steps=len(kuantan_export_test))
predict_mean = forecast_model.predicted_mean
plt.figure(figsize=(10,6))
plt.plot(kuantan_export['Date'],kuantan_export['Export(Kuantan)'],label='Training Set')
plt.plot(kuantan_export_test['Date'],kuantan_export_test['Export(Kuantan)'],label='Testing Set')
plt.plot(kuantan_export_test['Date'], predict_mean,label='Forecast')
...
#check accuracy
actual_data = np.array(kuantan_export_test['Export(Kuantan)'])
...
#Plot time series
plt.figure(figsize=(10,6))
plt.plot(kuantan_export['Date'],kuantan_export['Export(Kuantan)'],label='Original Data')
