...
#Filter Export(Port Dickson)
portd_export = cargo_throughput_data.loc[:,cargo_throughput_data.columns.isin(['Date','Export(Port Dickson)'])].reset_index(drop=True)
portd_export.head()
print("Export value in Port Dickson")
print(portd_export)

#plot original time series data
plt.figure(figsize=(10,6))
plt.plot(portd_export['Date'], portd_export['Export(Port Dickson)'])
...
portd_export_train = portd_export[:round(len(portd_export)*70/100)]
portd_export_test = portd_export[round(len(portd_export)*70/100):]

#testing stationarity
test_stationarity(portd_export,'Export(Port Dickson)')

#autocorrelation factor
fig,ax= plt.subplots(2, figsize=(10,6))
ax[0] = plot_acf(portd_export['Export(Port Dickson)'].dropna(), ax=ax[0], lags = 15)
ax[1] = plot_pacf(portd_export['Export(Port Dickson)'].dropna(), ax=ax[1], lags = 15)
plt.show()

#decomposition
portd_export['Date'] = pd.to_datetime(portd_export['Date'])
portd_export_newdf = portd_export.set_index('Date')
decomp = sm.tsa.seasonal_decompose(portd_export_newdf['Export(Port Dickson)'],model='additive')
decomp.plot()
plt.show()
...
for pm in pdq:
    for pm_seasonal in seasonal_pdq:
        try:
            model = sm.tsa.statespace.SARIMAX(portd_export_train['Export(Port Dickson)'],
                                              order=pm,
...
model_portd_export = sm.tsa.statespace.SARIMAX(portd_export_train['Export(Port Dickson)'],order=(1,0,1),seasonal_order=(0,1,1,12),
                                  enforce_stationarity=False,enforce_invertibility=False)
result = model_portd_export.fit()
...
portd_export_test['Date'] = pd.to_datetime(portd_export_test['Date'])
forecast_model = result.get_forecast(steps=len(portd_export_test))
predict_mean = forecast_model.predicted_mean
plt.figure(figsize=(10,6))
plt.plot(portd_export['Date'],portd_export['Export(Port Dickson)'],label='Training Set')
plt.plot(portd_export_test['Date'],portd_export_test['Export(Port Dickson)'],label='Testing Set')
plt.plot(portd_export_test['Date'], predict_mean,label='Forecast')
...
#check accuracy
actual_data = np.array(portd_export_test['Export(Port Dickson)'])
...
#Plot time series
plt.figure(figsize=(10,6))
plt.plot(portd_export['Date'],portd_export['Export(Port Dickson)'],label='Original Data')
