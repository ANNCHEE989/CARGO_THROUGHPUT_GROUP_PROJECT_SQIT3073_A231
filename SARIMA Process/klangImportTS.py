...
#Filter Import(Klang)
klang_import = cargo_throughput_data.loc[:,cargo_throughput_data.columns.isin(['Date','Import(Klang)'])].reset_index(drop=True)
klang_import.head()
print("Import value in Klang")
print(klang_import)

#plot original time series data
plt.figure(figsize=(10,6))
plt.plot(klang_import['Date'], klang_import['Import(Klang)'])
plt.title('Cargo import value in Klang')
plt.xlabel('Date')
plt.ylabel('\'000 Tan Metrik(Freightweight)')
plt.show()

#set traning and testing set
klang_import_train = klang_import[:round(len(klang_import)*70/100)]
klang_import_test = klang_import[round(len(klang_import)*70/100):]

#testing stationarity
test_stationarity(klang_import,'Import(Klang)')

#autocorrelation factor
fig,ax= plt.subplots(2, figsize=(10,6))
ax[0] = plot_acf(klang_import['Import(Klang)'].dropna(), ax=ax[0], lags = 15)
ax[1] = plot_pacf(klang_import['Import(Klang)'].dropna(), ax=ax[1], lags = 15)
plt.show()

#decomposition
klang_import['Date'] = pd.to_datetime(klang_import['Date'])
klang_import_df = klang_import.set_index('Date')
decomp = sm.tsa.seasonal_decompose(klang_import_df['Import(Klang)'],model='additive')
decomp.plot()
plt.show()
...
for pm in pdq:
    for pm_seasonal in seasonal_pdq:
        try:
            model = sm.tsa.statespace.SARIMAX(klang_import_train['Import(Klang)'],
                                              order=pm,
...
model_klang_import = sm.tsa.statespace.SARIMAX(klang_import_train['Import(Klang)'],order=(0,1,1),seasonal_order=(0,1,1,12),
                                  enforce_stationarity=False,enforce_invertibility=False)
result = model_klang_import.fit()
...
klang_import_test['Date'] = pd.to_datetime(klang_import_test['Date'])
forecast_model = result.get_forecast(steps=len(klang_import_test))
predict_mean = forecast_model.predicted_mean
plt.figure(figsize=(10,6))
plt.plot(klang_import['Date'],klang_import['Import(Klang)'],label='Training Set')
plt.plot(klang_import_test['Date'],klang_import_test['Import(Klang)'],label='Testing Set')
plt.plot(klang_import_test['Date'], predict_mean,label='Forecast')
plt.title('Forecast Model of Klang Cargo Import Value')
plt.xlabel('Date')
plt.ylabel('\'000 Tan Metrik(Freightweight)')
plt.legend()
plt.show()
...
#check accuracy(RMSE)
actual_data = np.array(klang_import_test['Import(Klang)'])
...
#Plot time series
plt.figure(figsize=(10,6))
plt.plot(klang_import['Date'],klang_import['Import(Klang)'],label='Original Data')
plt.plot(future_df['Date'],future_df['Prediction'],label='Forecast')
plt.title('Future Cargo Import Value Prediction')
plt.xlabel('Date')
plt.ylabel('\'000 Tan Metrik(Freightweight)')
plt.legend()
plt.show()